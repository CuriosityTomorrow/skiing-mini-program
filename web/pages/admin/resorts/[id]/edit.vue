  ...
  <div class="min-h-screen bg-gray-50">
    <div class="max-w-3xl mx-auto px-4 py-8">
      <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">{{ isEdit ? '编辑滑雪场' : '新增滑雪场' }}</h1>
      </div>
...
const isEdit = route.params.id !== 'new' && !!route.params.id
...
if (isEdit) {
  const { data } = await useFetch(`/api/resorts/${route.params.id}`)
  if (data.value?.data) {
    const r = data.value.data
    form.value = {
      name: r.name, province: r.province, city: r.city, district: r.district || '',
      address: r.address || '', type: r.type, rating: r.rating, popularity: r.popularity,
      latitude: r.latitude, longitude: r.longitude,
      trails: r.trails || { total: 0, beginner: 0, intermediate: 0, advanced: 0, expert: 0, maxLength: 0, verticalDrop: 0 },
      pricing: r.pricing || { weekdayDaily: 0, weekendDaily: 0 },
      season: r.season || { yearRound: false, openMonth: 11, closeMonth: 3 },
    }
    imagesText.value = (r.images || []).join('\n')
  }
}

async function save() {
  saving.value = true
  const payload = { ...form.value, images: imageList.value }
  if (isEdit) {
    await $fetch(`/api/admin/resorts/${route.params.id}`, { method: 'PUT', body: payload })
  } else {
    await $fetch('/api/admin/resorts', { method: 'POST', body: payload })
  }
  saving.value = false
